...
Optional:
- `LLM_API_KEY`, `LLM_API_BASE_URL`, `LLM_MODEL` — any OpenAI-compatible chat-completions provider, used only to write place descriptions for POIs not covered by Wikipedia. Recommended: Google Gemini free tier (`LLM_API_BASE_URL=https://generativelanguage.googleapis.com/v1beta/openai/`, `LLM_MODEL=gemini-flash-latest` — an alias that stays current; older ids like `gemini-2.5-flash` 404 for new keys). Descriptions resolve in tiers: cache → Wikipedia extract → LLM → varied per-type templates, so **without any LLM key the app still works** (templates only). Legacy `NEBIUS_API_KEY` / `NEBIUS_API_BASE_URL` are used as a fallback when the `LLM_*` vars are unset.
