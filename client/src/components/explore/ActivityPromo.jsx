import { civitatisCityUrl } from '../../services/affiliates';
import { track } from '../../services/analytics';

// Affiliate promo for tours/tickets in the routed city, shown under the
// stops list. Renders nothing unless VITE_CIVITATIS_AID is configured.
...
export default function ActivityPromo({ city }) {
  const url = civitatisCityUrl(city);
  if (!url) return null;
  ...
  return (
    <section className="xp-promo" aria-label={`Actividades en ${city}`}>
      <a
        ...
        onClick={() => track('civitatis_clicked', { city, placement: 'route' })}
      >
        ...
        <span className="xp-promo-text">
          ...
          <span className="xp-promo-sub">Tours, tickets sin colas y free tours con Civitatis</span>
        </span>
